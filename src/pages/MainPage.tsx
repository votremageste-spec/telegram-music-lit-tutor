...
export const MainPage: React.FC<MainPageProps> = ({ onNavigate, userProfile }) => {
  ...
  const getSpecializationName = () => {
    if (userProfile?.specialization_ru) {
      return userProfile.specialization_ru;
    }

    if (userProfile?.specialization === 'instrumentalist') {
      return 'Инструменталист / Хоровик';
    }

    if (userProfile?.specialization === 'vocalist') {
      return 'Вокалист';
    }

    if (userProfile?.specialization === 'folklore') {
      return 'Фольклор';
    }

    return 'Специализация не выбрана';
  };
  ...
  const menuCards = [
    {
      id: 'composers',
      title: '📚 Композиторы',
      description: 'Изучай биографии, произведения и слушай музыку',
    },
    {
      id: 'audio',
      title: '🎧 Аудиовикторина',
      description: 'Узнавай произведения на слух',
    },
    {
      id: 'tickets',
      title: '📝 Экзаменационные билеты',
      description: 'Готовься по реальным билетам',
    },
    {
      id: 'tests',
      title: '✍️ Тесты',
      description: 'Проверяй свои знания',
    },
    ...(userProfile?.program === 'FGT'
      ? [
          {
            id: 'project',
            title: '🎯 Реферат / проект',
            description: 'Темы рефератов',
            action: () =>
              alert('Темы рефератов: И.С. Бах, Венская классическая школа и др.'),
          },
        ]
      : []),
    {
  ...
  return (
    <div className="main-page">
      <div className="welcome-section">
        <h1>Привет, {user?.first_name || userProfile?.first_name || 'ученик'}!</h1>

        <div className="user-info-card">
          <p>📖 {userProfile?.program_ru}</p>
          <p>🎵 {getSpecializationName()}</p>
      ...
      <div className="menu-grid">
        {menuCards.map((card) => (
          <button
            key={card.id}
            className="menu-card"
            onClick={() => ('action' in card ? card.action() : onNavigate(card.id))}
          >
